...
def reinforce(args):
    ...
    agent.actor_critic.load_state_dict(torch.load("VPG_MountainCar.h5"))

    # summary(agent.actor_critic.pi, (obs_dim, ))
    # summary(agent.actor_critic.v, (obs_dim, ))
...
#             traj_reward_list = []
#
#             log_file.append([epoch, pi_loss, q_loss, average_reward])
#
#     log2file_ddpg(log_file, "DDPG", "MountainCar")
#     torch.save(agent.actor_critic.state_dict(), "DDPG_MountainCar.h5")
#     env.close()
...
def ddpg_new(args):
    ...
    new_DDPG_agent.actor.load_state_dict(torch.load("DDPG_MountainCar.h5"))
    ...
    while not done or steps < 1000:
        ...
        next_obs, reward, d, _ = env.step(act)
        env.render()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_trajectories', '-nt', type=int, default=20000)
